...
export const Login = () => {
  ...
  // Use a single input for email/username/mobile
  const [identifier, setIdentifier] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  // Initialize Keycloak once on component mount
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login({ identifier, password }); // optional if you still want local login
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="john_doe / john@email.com / +254712345678"
              required
            />
          </div>
